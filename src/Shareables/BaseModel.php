...
abstract class BaseModel extends Model
{
	...
	abstract protected static function relationship();
	abstract protected static function methods();
	abstract protected static function attSet();
	abstract protected static function attGet();
    ...
    {
		if(isset($this->attSet()[$method])) {
			$this->{$this->attSet()[$method]} = $parameters[0];
			return $this;
		} elseif(isset($this->attGet()[$method])) {
            return $this->{$this->attGet()[$method]};
        } elseif(isset($this->methods()[$method])) {
			$data = $this->methods();
			return $this->{$data[$method]}->get();//aqui vai depender se é hasMany, belongsTo, etc..
		} else {
	...
	{
		if(isset($this->relationship()[$key])) {
			$data = $this->relationship();
			return $this->{$data[$key]['rel']}($data[$key]['model'], $data[$key]['foreign_col']);
		}
